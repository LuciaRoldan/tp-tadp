    ...
    def apply(contrincantes: Contrincantes): Contrincantes = {
      ...
      if (atacante.tieneItem(item)) {
        (item, atacante, atacado) match {
          ...
          case (armaDeFuego, atacante: Biologico, atacado: Humano) => if (atacante.tieneMunicion) {
            (atacante.perderMunicion(), atacado.cambiarKi(atacado.ki - 20))
          }
          else (atacante, atacado)
          case (armaDeFuego, atacante, atacado: Namekusein) => if (atacante.tieneMunicion) {
            (atacante.perderMunicion(), atacado.cambiarKi(atacado.ki - 10))
          }
          else (atacante, atacado)
        }
      }
